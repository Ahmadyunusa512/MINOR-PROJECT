document.addEventListener("DOMContentLoaded", () => {
    ...
    if (redeemBtn) {
        redeemBtn.addEventListener("click", function() {
            ...
            const pointsToRedeem = parseInt(loyaltyInput?.value || 0);
    ...
    function redeemLoyaltyPoints(points) {
        ...
        if (!loyaltyMessage || !loyaltyInput) return;

        // Validate input
        if (!points || points <= 0) {
            loyaltyMessage.textContent = "Please enter points to redeem";
            loyaltyMessage.classList.remove("success", "error");
            return;
        }

        // Check user points available
        const user = getLoggedInUser();
        const availablePoints = user && user.points ? user.points : 0;

        if (points > availablePoints) {
            loyaltyMessage.textContent = `✗ Only ${availablePoints} points available`;
            loyaltyMessage.classList.add("error");
            loyaltyMessage.classList.remove("success");
            return;
        }

        // Calculate discount from points (1000 points = ₦1000 discount)
        const discountAmount = (points / 1000) * 1000;

        // Cannot exceed cart total
        if (discountAmount > subtotal) {
            loyaltyMessage.textContent = `✗ Discount exceeds cart total`;
            loyaltyMessage.classList.add("error");
            loyaltyMessage.classList.remove("success");
            return;
        }
        ...
        loyaltyMessage.textContent = `✓ Redeemed ${points} points! Save ${CURRENCY}${discountAmount.toFixed(2)}`;
        loyaltyMessage.classList.add("success");
        loyaltyMessage.classList.remove("error");

        // Deduct points from user
        if (user) {
            user.points = (user.points || 0) - points;
            saveUser(user);
            updateLoyaltyPoints(subtotal - discountAmount);
        }
    ...
    if (myCartSidebar && liveChatWidget) {
        ...
        // Hide live chat when user scrolls near the bottom of page (to avoid overlapping footer)
        window.addEventListener("scroll", () => {
            ...
            if (scrollFromBottom < 300) {
                // User is near bottom of page - hide chat
                liveChatWidget.classList.add("hidden");
            } else {
                // User is not near bottom - show chat (unless cart is open)
                if (!myCartSidebar.classList.contains("active")) {
                    liveChatWidget.classList.remove("hidden");
                }
            }
